...
- New **Output panel**: finished files appear in a panel below the input
  list (separated by a draggable divider) with a built-in preview - videos
  play with audio (QtMultimedia), images, PDF pages and PPTX slides render
  inline; Open / Show in Folder / Clear List actions. It opens when the
  first file finishes and can be shown/hidden from the toolbar
